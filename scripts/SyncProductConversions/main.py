...
def main():
    logging.debug("Updating and creating the product conversions")

    endpoint = "/api/products/conversions/sync-primavera"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response_data.result_code <= 0:
                logging.error(f"Failed to update product conversions. Error: {response_data.result_code} - {response_data.result}")
            else:
                logging.info(f"Product conversions updated sucessfully: {response_data}")
        else:
            logging.error(f"Failed to update product conversions. Error: {response_data.get('Message')}")

    except requests.exceptions.RequestException as e:
        logging.error(f"Error updating product conversions: {e}, \nwith response status: {response.status_code} \nand response text: {response.text}")
