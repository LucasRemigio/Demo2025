...
def main():
    logging.debug("Updating and creating the clients")

    endpoint = "/api/clients/sync-primavera"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response_data.result_code <= 0:
                logging.error(f"Failed to update clients. Error: {response_data.result_code} - {response_data.result}")
            else:
                logging.info(f"Clients updated sucessfully: {response_data}")
        else:
            logging.error(f"Failed to update clients. Error: {response_data.get('Message')}")
